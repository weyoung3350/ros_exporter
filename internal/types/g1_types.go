...
func (sdk *G1SDK) Initialize(configPath string) error {
	...
	result := C.g1_sdk_init(cConfigPath)
	if result != 0 {
		errMsg := sdk.getLastError()
		return fmt.Errorf("SDK初始化失败: %s", errMsg)
	}
...
func (sdk *G1SDK) Connect() error {
	...
	result := C.g1_sdk_connect()
	if result != 0 {
		errMsg := sdk.getLastError()
		return fmt.Errorf("连接失败: %s", errMsg)
	}
...
func (sdk *G1SDK) Disconnect() error {
	...
	result := C.g1_sdk_disconnect()
	if result != 0 {
		errMsg := sdk.getLastError()
		return fmt.Errorf("断开连接失败: %s", errMsg)
	}
...
func (sdk *G1SDK) GetBatteryStatus() (*G1BatteryStatus, error) {
	...
	result := C.g1_sdk_get_battery_status(&cStatus)
	if result != 0 {
		errMsg := sdk.getLastError()
		return nil, fmt.Errorf("获取电池状态失败: %s", errMsg)
	}
	
	// 转换C结构体到Go结构体
	status := &G1BatteryStatus{
		Voltage:       float64(cStatus.voltage),
		Current:       float64(cStatus.current),
		Temperature:   float64(cStatus.temperature),
		Capacity:      float64(cStatus.capacity),
		CycleCount:    uint32(cStatus.cycle_count),
		IsCharging:    bool(cStatus.is_charging),
		IsDischarging: bool(cStatus.is_discharging),
		HealthStatus:  uint8(cStatus.health_status),
		ErrorCode:     uint32(cStatus.error_code),
		ErrorMessage:  C.GoString(&cStatus.error_message[0]),
		Timestamp:     time.Unix(0, int64(cStatus.timestamp)*int64(time.Millisecond)),
	}
	
	// 转换单体电压数组
	status.CellVoltages = make([]float64, 40)
	for i := 0; i < 40; i++ {
		status.CellVoltages[i] = float64(cStatus.cell_voltages[i])
	}
	
	// 转换温度数组
	status.Temperatures = make([]float64, 12)
	for i := 0; i < 12; i++ {
		status.Temperatures[i] = float64(cStatus.temperatures[i])
	}
...
func (sdk *G1SDK) getLastError() string {
